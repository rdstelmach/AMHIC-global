...
starting_pop_data <- #starting_data
  subset(starting_data, 
                            country_sex_age %in% 
                              c("United Kingdom_Female_14",
                                "Afghanistan_Male_17"))
...
#input_files <- grep("no_interventions|school_based", input_files, value = TRUE,
#                    invert = TRUE)

# select outcome(s) of interest-------------------------------------------------
effects_var <- c("dalys", "benefits") # options: benefits, dalys
cases <- "Base" # options: base

# summary functions for results
summarize_results <- function (summary_var, group_label) {
  res_subset = model_values_subanalysis %>%
    group_by_(summary_var, ".strategy_names", "value_names") %>%
    summarize(value = sum(value)) %>%
    pivot_wider(id_cols = c(summary_var),
                names_from = c(".strategy_names", "value_names"))
  res_subset$roi = ((res_subset$invest_benefit - res_subset$base_benefit) - 
                      res_subset$invest_cost) / res_subset$invest_cost
  res_subset$cost_per_daly_averted = res_subset$invest_cost / 
    (res_subset$base_dalys - res_subset$invest_dalys)
  
  names(res_subset)[1] <- "subgroup"
  names(res_subset)[2:ncol(res_subset)] <- 
    paste0(names(res_subset)[2:ncol(res_subset)], "_exp")
  
  res_subset = bind_cols(group = group_label,
                         res_subset)
  res_subset
}

# run model---------------------------------------------------------------------
# creates abbreviated output for checking model edits
#res_out <- data.frame() 

file_path_model_define <-
  "Model definition files/AUTO GENERATED cleaned model inputs/"

for (k in 1:length(cases)) {
for (j in 1:length(input_files)) {
  excel_sheet_with_data <- paste0("Model definition files/", input_files[j])
  ...
  for (m in 1:length(effects_var)) {
    ...
    # run whole model
    model_counts <- vector("list", nrow(starting_pop_data))
    model_values <- vector("list", nrow(starting_pop_data))
    
    names(model_counts) <- starting_pop_data$country_sex_age
    names(model_values) <- starting_pop_data$country_sex_age
    
    for (i in 1:nrow(starting_pop_data)) {
      ...
      model_counts[[i]] <- get_counts(res_mod)
      model_values[[i]] <- get_values(res_mod)
    }
    
    ## summarize_output
    
    # get the number of people in each state in each run of the model
    for (i in 1:length(model_counts)) {
      model_counts[[i]]$country_sex_year <- names(model_counts[i])
    }
    ...
    if (effects_var[m] == "benefits") {
    write.csv(model_counts, row.names = FALSE,
              paste0("Model output files/", cases[k], " case/Raw values/", 
                     cases[k], " case raw model counts ", 
                     gsub("amh_model_inputs_|.xlsx", "",
                          input_files[j]),
                     format(Sys.time(), "_%Y%m%d_%H%M"), 
                     ".csv"))
    write.csv(model_values_cb, row.names = FALSE,
              paste0("Model output files/", cases[k], " case/Raw values/", 
                     cases[k], " case raw model costs #benefits ", 
                     gsub("amh_model_inputs_|.xlsx", "",
                          input_files[j]),
                     format(Sys.time(), "_%Y%m%d_%H%M"), 
                     ".csv"))
  ...
  model_values_subanalysis[, c("country", "sex", "age_at_start")] <-
    str_split_fixed(model_values_subanalysis$country_sex_year, "_", 3)
  model_values_subanalysis <- merge(model_values_subanalysis, wb_and_who)
  
  model_values_subanalysis$total <- "Total"
  
  res_total <- summarize_results("total", "Total")
  res_wb    <- summarize_results("wb_income_level", "World Bank income level")
  res_who   <- summarize_results("who_region_name", "WHO region name")
  res_sex   <- summarize_results("sex", "Sex")
  res_age   <- summarize_results("age_at_start", "Age at start")
  
  res_all <- bind_rows(res_total, res_wb, res_who, 
                        res_sex, res_age)
  
  write.csv(res_all, row.names = FALSE,
            paste0("Model output files/", cases[k], " case/", cases[k], " case_", 
                   gsub("amh_model_inputs_|.xlsx", "",
                        input_files[j]),
                   format(Sys.time(), "_%Y%m%d_%H%M"), 
                   ".csv"))
...
# play the Mario theme tune when the code's done running------------------------
beep("mario")
